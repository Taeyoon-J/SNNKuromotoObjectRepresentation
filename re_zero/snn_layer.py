...
class SNNLayer(nn.Module):
    ...
    def __init__(
        ...
        self.membrane_weight = nn.Parameter(torch.full((self.num_pixels, osc_dim), 0.5))
    ...
    def forward_step(
        ...
        Shapes:
            membrane_prev: [B, H*W]
            spikes_prev: [B, H*W]
            sinusoidal_gate: [B, H*W, 1]
            gamma: [B, H*W, D]
        """
        sinusoidal_wave = sinusoidal_gate * gamma
        synaptic_drive = (sinusoidal_wave * self.membrane_weight.unsqueeze(0)).sum(dim=-1)
