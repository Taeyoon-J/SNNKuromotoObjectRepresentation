...
from SNN.SNN_layers.spike_dense import *
from SNN.SNN_layers.spike_neuron import *
from SNN.SNN_layers.spike_rnn import *
...
class rnn_test_bold(nn.Module):
    def __init__(self, T=500, low_n=0, high_n=4, branch=4):
        super(rnn_test_bold, self).__init__()

        self.input_size = input_size
        self.T = T
        self.rnn_1 = Rhy_spike_rnn_test_denri_wotanh_R(
            self.input_size, cfg_fc[0], 
            tau_minitializer='uniform', low_m=0, high_m=0,
            tau_ninitializer='uniform', low_n=low_n, high_n=low_n, 
            vth=thresh, branch=branch,
            dt=1, cycle_min=cycle_min[0], cycle_max=cycle_max[0], 
            duty_cycle_min=duty_cycle_min[0], duty_cycle_max=duty_cycle_max[0], 
            phase_max=phase_max[0], T=T,
            device=device, bias=True
        )
    ...
    def forward(self, input):
        ...
        for i in range(seq_num):
            ...
            spikes_list.append(spike_layer1)
            mem_list.append(mem_layer1)


        outputs = torch.stack(spikes_list).permute(1, 2, 0) 
        return torch.stack(mem_list).permute(1, 2, 0)
        # return outputs
...
class rnn_test2(nn.Module):
    def __init__(self, T=500, low_n=0, high_n=4, branch=4):
        super(rnn_test2, self).__init__()

        self.input_size = input_size 
        self.output_size = output_size
        self.T = T

        self.rnn_1 = Rhy_spike_rnn_test_denri_wotanh_new(
        ...
        self.dense_2 = spike_dense_test_origin(
            cfg_fc[0], 
            self.output_size,
            vth=0.5, 
            dt=1, 
    ...
    def forward(self, input, external_masks=None):
        """
        Args:
            input: [Batch, T, Input_Dim]
            external_masks: [Batch, T, Hidden_Dim] 
        """
        batch_size, seq_num, input_dim = input.shape
        ...
        for i in range(seq_num):
            ...
            if external_masks is not None:
                current_mask = external_masks[:, i, :]
            else:
                current_mask = None

            mem_layer1, spike_layer1 = self.rnn_1.forward(input_x, current_mask)
...
class rnn_test3(nn.Module):
    def __init__(self, T=500, low_n=0, high_n=4, branch=4):
        super(rnn_test3, self).__init__()
        self.input_size = input_size 
        self.output_size = output_size
        self.T = T
    ...
    def forward(self, input, external_masks=None):
        ...
        for i in range(seq_num):
            ...
            mem_layer2, spike_layer2 = self.dense_2.forward(spike_layer1)
            spikes_hist.append(spike_layer1)
